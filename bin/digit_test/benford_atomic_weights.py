...
def digit_test(input_data, mode):
    ...
    digit_frequency = [(float(x) / 100) * 91 for x in input_data]
    print(input_data)
    ...
    #compute chi-squared
    chi_squared = compute_chi_squared_statistic(benford_frequency, digit_frequency)

    return(digit_frequency, benford_frequency, digit_frequency_percent, benford_frequency_percent, z_stat, von_mises_stat, d_star_stat, chi_squared)
...
def benford_finite_range(input_data, mode, lb, ub):
    ...
    return(digit_frequency, benford_frequency, digit_frequency_percent, benford_frequency_percent, z_stat, von_mises_stat, d_star_stat, chi_squared, dataset_size)
...
def compute_chi_squared_statistic(expected_list, actual_list):
    ...
    return_value = [chi, chi/(len(expected_list) - 1)]
...
def plot_bar_chart(bins, frequency, benford_freq, dataset_size, von_mises, dstar, chi_squared, mode):
    stats = "chi_squared"
    ...
    if mode in ['1', 'f1']:
        ax0.errorbar(bins, frequency, yerr=yerror, label="Atomic Wt. Occurrence", color='black', marker='x', fmt='x', capsize=3, elinewidth=1, zorder=1)
    elif mode == '12':
    ...
    if stats == "chi_squared":
        ...
        patch.append(mpatches.Patch(color='none', label=r'$\chi^2_\nu = {}$'.format(round(chi_squared,1))))
...
def main(mode):
    ...
    if mode in ['1', '12', '12h', '12hn', '23', '23h', '23hn', '2']:
        data_raw, benford_raw, data_percent, benford_percent, z_statistic, von_mises_statistic, d_star_statistic, chi_squared = digit_test(data, mode)
        ...
        data_raw, benford_raw, data_percent, benford_percent, z_statistic, von_mises_statistic, d_star_statistic, chi_squared, data_size = benford_finite_range(data, mode, lowerbound, upperbound)
        bins_to_plot = np.arange(9)
    ...
    if mode in ['1','f1','f2','2','12']:
        plot_bar_chart(bins_to_plot, data_raw, benford_raw, data_size, von_mises_statistic[2], d_star_statistic[1], chi_squared[1], mode)
